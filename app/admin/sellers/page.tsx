...
import { getSellers, createSeller, updateSeller, toggleSeller, deleteSeller } from '@/app/lib/api';
...
export default function AdminSellers() {
  ...
  const handleOpenModal = (seller?: any) => {
    if (seller) {
      setForm({ id: seller.id, username: seller.username, email: seller.email, phone: seller.phone || '', password: '' });
    } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (form.id) {
        ...
        await updateSeller(form.id, payload);
        setToast({ message: 'Seller updated successfully', type: 'success' });
      } else {
        await createSeller({ username: form.username, email: form.email, phone: form.phone, password: form.password });
  ...
  const handleToggle = async (id: number) => {
    try {
      ...
      fetchSellers();
      setToast({ message: 'Seller status updated', type: 'success' });
    } catch (err: any) {
      setToast({ message: err.message, type: 'error' });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSeller(confirmModal.id);
      setToast({ message: 'Seller deleted successfully', type: 'success' });
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {sellers.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-4 text-center text-sm text-gray-500">No sellers found</td></tr>
              ) : (
                sellers.map((seller: any) => (
                  <tr key={seller.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{seller.id}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{seller.username}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{seller.email}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${seller.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {seller.is_active ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                      <button onClick={() => handleToggle(seller.id)} className="text-blue-600 hover:text-blue-900">Toggle</button>
                      <button onClick={() => handleOpenModal(seller)} className="text-orange-600 hover:text-orange-900">Edit</button>
                      <button onClick={() => setConfirmModal({ isOpen: true, id: seller.id, isLoading: false })} className="text-red-600 hover:text-red-900">Delete</button>
                    </td>
                  </tr>
                ))
              )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <h3 className="text-xl font-bold text-gray-900">{form.id ? 'Edit Seller' : 'Add Seller'}</h3>
